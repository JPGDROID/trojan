...
start_docker(){
	...
	docker run --name trojan --restart=always -d -p 50:50 -p 445:445 -v /usr/src/trojan:/usr/src/trojan  atrandys/trojan sh -c "/etc/init.d/nginx start && trojan -c /usr/src/trojan/server.conf"
	echo "============================"
	echo "       trojan启动完成"
	echo "============================"
}
